...
import numpy as np
from covebilstm import CoveBiLSTM
from crf import CRF


class Cove_BiLSTM_CRF(nn.Module):
    def __init__(self, data):
        super(Cove_BiLSTM_CRF, self).__init__()
        print "build batched covelstmcrf..."
        self.gpu = data.HP_gpu
        self.average_batch = data.HP_average_batch_loss
        ## add two more label for downlayer lstm, use original label size for CRF
        label_size = data.label_alphabet_size
        data.label_alphabet_size += 2
        self.lstm = CoveBiLSTM(data)
        self.crf = CRF(label_size, self.gpu)

    def neg_log_likelihood_loss(self, word_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover,
                                batch_label, mask):
        outs = self.lstm.get_output_score(word_inputs, word_seq_lengths, char_inputs, char_seq_lengths,
                                          char_seq_recover)
    ...
    def get_lstm_features(self, word_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
        return self.lstm.get_lstm_features(word_inputs, word_seq_lengths, char_inputs, char_seq_lengths,
                                           char_seq_recover)
